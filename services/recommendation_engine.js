        ...
        for (const userId of allUserIds) {
            ...
            if (candidateType) {
                ...
                if (rankedProducts.length > 0) {
                    ...
                    const keyboard = {
                        inline_keyboard: [[{ text: '🛍️ Ouvrir la Mini App', web_app: { url: (process.env.RENDER_EXTERNAL_URL ? `${process.env.RENDER_EXTERNAL_URL}/catalog?v=${Date.now()}` : (process.env.RAILWAY_PUBLIC_DOMAIN ? `https://${process.env.RAILWAY_PUBLIC_DOMAIN}/catalog?v=${Date.now()}` : `https://monshopbot-production.up.railway.app/catalog?v=${Date.now()}`)) } }]]
                    };
