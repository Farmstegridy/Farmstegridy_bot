        ...
        for (const user of users) {
            ...
            if (currentHour === targetHour && lastReminder !== today) {
                ...
                bot.telegram.sendMessage(tgId, message, {
                    parse_mode: 'HTML',
                    reply_markup: {
                        inline_keyboard: [[{ text: '🚀 Ouvrir la Mini App', web_app: { url: (process.env.RENDER_EXTERNAL_URL ? `${process.env.RENDER_EXTERNAL_URL}/catalog?v=${Date.now()}` : (process.env.RAILWAY_PUBLIC_DOMAIN ? `https://${process.env.RAILWAY_PUBLIC_DOMAIN}/catalog?v=${Date.now()}` : `https://monshopbot-production.up.railway.app/catalog?v=${Date.now()}`)) } }]]
                    }
                }).catch(() => {});
    ...
    try {
        ...
        for (const userId in allCarts) {
            ...
            if (now - c.updated_at > 15 * 60 * 1000 && !c.notified) {
                ...
                const keyboard = {
                    inline_keyboard: [[{ text: '✅ Finaliser ma commande', web_app: { url: (process.env.RENDER_EXTERNAL_URL ? `${process.env.RENDER_EXTERNAL_URL}/catalog?v=${Date.now()}` : (process.env.RAILWAY_PUBLIC_DOMAIN ? `https://${process.env.RAILWAY_PUBLIC_DOMAIN}/catalog?v=${Date.now()}` : `https://monshopbot-production.up.railway.app/catalog?v=${Date.now()}`)) } }]]
                };
